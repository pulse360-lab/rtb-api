...
const saveOnChage = async(userId, locale) => 
    await global.redis.save(`user-location:${param.from.id}`, locale);
...
//Using Latitude and Longitude to get the current localization.
 const get = async (req, res) =>{
    ...
    await saveOnChage(msg, {city: result.address.city,  latitude: req.query.latitude, longitude: req.query.longitude});
    res.json(result);
 }
